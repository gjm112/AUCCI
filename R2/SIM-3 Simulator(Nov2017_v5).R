...
param1 = list(alphabet = data.frame(phi = rep(c(.5,.7), each=4), 
              ...
              gamma = data.frame(q1 = c(.85, .9), q2 = c(.90, .95), q3=c(.85, .9), q4=c(.90, .95), gamma = c(.9, .95), rho = c(.5, .7)))
...
for (i in d1) {
  for (j in d2) {
if ((i ==3 & j==1)) {    # break and resume by controlling numbers
    ...
    saveRDS(temp.d3, paste0("R2/Simdata/sim_data","-",format(Sys.time(), "%b%d"),"-",i,j,".rds"))
    # Time stat
    elapsed = Sys.time()-bgn
