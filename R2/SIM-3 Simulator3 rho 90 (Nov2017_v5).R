### Part II. Simulation
# RHO = 90% //  Theta = 80, 90, 95, 99% //  MI = complete, naive, MICE (not NORM)
########################################
...
source("R/CI-5 AUCCI_MI (Incomplete).R")
source("R/SIM-1 DataGenerator(DTVR).R")
source("R/SIM-2 Evaluator.R")
...
# RHO = 90%  (w/ theta= 0.8, 0.9, 0.95, 0.99)
param1 = list(alphabet = data.frame(phi = rep(c(.5,.7), each=4), 
              ...
              gamma = data.frame(q1 = .99, q2 = .99, q3 = .99, q4=.99, gamma = .95, rho = .90))
...
MI.methods = data.frame(functions=rep("mice2",2), methods=c("pmm","logreg"))
...
#    names(temp.eval)[3:7] <- paste0("theta=",round(theta,2),", phi=",phi,", rho=",rho,", n=",n[h],c(", 2.pmm", ", 3.logreg", ", 4.simple", ", 5.coinflip", ", 6.adaptive"))
    names(temp.eval)[3:4] <- paste0("theta=",round(theta,2),", phi=",phi,", rho=",rho,", n=",n[h],c(", 2.pmm", ", 3.logreg"))
  } #imputation==TRUE
  if (Dir==TRUE) {
...
for (i in d1) {
  for (j in d2) {
 if ((i > 5)) {    # break and resume by controlling numbers
    if (parallel) {cl <- makeSOCKcluster(3)}      # for parallel
    ...
    saveRDS(temp.d3, paste0("R2/Simdata/sim_data","-",format(Sys.time(), "%b%d"),"-",i,"3.rds"))
    # Time stat
    elapsed = Sys.time()-bgn
