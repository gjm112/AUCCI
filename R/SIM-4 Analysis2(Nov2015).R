...
names(rst)[7:9] <- paste0("v.",names(rst)[7:9])

## aggregating across phi, rho, n:  table(CI,MI,measure x theta)
rst.avg <- aggregate(cbind(v.0.8,v.0.9,v.0.95) ~ CI.method + MI + measure, mean, data=rst)
aggregate(cbind(v.0.8,v.0.9,v.0.95) ~ CI.method + MI + measure + phi, mean, data=rst)


# Calculate Mean Absolute Error for CP only and combine with rst.avg
rst.MAE <- aggregate(cbind(v.0.8,v.0.9,v.0.95) ~ CI.method + MI + measure, MAE, data=rst[rst$measure=="CP",])
...
aggregate(cbind(0.8, 0.9, 0.95) ~ CI.method + MI, FUN=MAE, data=rst2[rst2$measure=="CP",])
