...
beta1 = 1.9767
a <- Sys.time(); temp <- datagenerator(n=100000, alpha0=alpha0, alpha1=alpha1, beta0=beta0, beta1=beta1, beta2=beta2, beta3=beta3, sig=sig, q1=q1, q2=q2, q3=q3, q4=q4, gamma=gamma, mu.V=mu.V, Sigma=Sigma, DT.only=TRUE) ; Sys.time()-a
a <- Sys.time(); AUC(temp$marker[temp$disease==0], temp$marker[temp$disease==1]); Sys.time()-a

# theta finder
a <- Sys.time(); n=10000000; n1=1000000; n2 = floor(n/n1)
set.seed(400)
for (k in 1:5) {
  tht = rep(NA,10); for (j in 1:10) {
    ...
    a <- Sys.time(); tht[j] <- AUC(temp$marker[temp$disease==0], temp$marker[temp$disease==1]); Sys.time()-a
  }
  print(tht)
}
