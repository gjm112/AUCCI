...
AUCCI.MI = function(data, MI.function, MI.method, score.MI = "fixed.r", m, CI.method, alpha=.05, LT=FALSE, variance=FALSE, MI.thetas=FALSE, ...) {
  ...
  ### C. CI stage
  {
    ...
    if (CI.method %in% CI.Wald) {
      ...
      if (CI.method == "NS1") {
        CI = multiroot(HMS.equation, start, AUC.hat=mean.MI, n.x=n.x, n.y=n.y, alpha=alpha, MI=mi.stat$theta.LT, LT=LT, score.MI=score.MI, sample.var=FALSE, ...)$root}
      else if (CI.method == "NS2"){
        CI = multiroot(NC.equation,  start, AUC.hat=mean.MI, n.x=n.x, n.y=n.y, alpha=alpha, MI=mi.stat$theta.LT, LT=LT, score.MI=score.MI, sample.var=FALSE, ...)$root}  
      else if (CI.method == "Mee") {
        CI = multiroot(Mee.equation,  start, AUC.hat=mean.MI, x=x, y=y, n.x=n.x, n.y=n.y, alpha=alpha, MI=mi.stat$theta.LT, LT=LT, score.MI=score.MI, sample.var=FALSE, ...)$root
      }
